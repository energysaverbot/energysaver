...
void phase1()
{
    ...
    for (int i = 1; i <= Time ; i++)
    {
        ...
        for (int j = 0; j < Task.size(); j++)
        {
            if (Task[j].arrival == i)
            {
                int diff = Task[j].deadline - i;
                maxheap.push({diff*(-1), {Task[j].id, Task[j].util}});
            }
        }
    ...
    for (int i = 0; i < Task.size(); i++)
    {
        ...
        if (istaskavailable(Task[i].arrival, Task[i].deadline, timeslot))
        {
            ...
            if ((taskscheduled[Task[i].id] == 0) && (Task[i].util <= available_util))
            {
                ...
                int diff = max(Task[i].deadline - timeslot,0);
                if (difffinal > diff)
                {
                    utl = Task[i].util;
                    difffinal = diff;
...
int main()
{
   ...
   //cout<<"\nphase 1 profit "<<phase1profit<<endl;
    cout<<"\nFinal Revenue->"<<totalProfit<<endl;
